...
with tab_hn:
    if r_hn is None:
        ...
        # Always calculate from full r_hn so cards always show all classifications
        r_hn_q = r_hn.copy(); r_hn_q['Quantity'] = pd.to_numeric(r_hn_q['Quantity'], errors='coerce').fillna(0)
        tot_all_h = int(r_hn_q['Quantity'].sum())
        reg_h  = int(r_hn_q[r_hn_q['Clasificacion']=='Regular']['Quantity'].sum())
        vmi_h  = int(r_hn_q[r_hn_q['Clasificacion']=='VMI']['Quantity'].sum())
        irr_h  = int(r_hn_q[r_hn_q['Clasificacion']=='Irregulares']['Quantity'].sum())
        obs_h  = int(r_hn_q[r_hn_q['Clasificacion']=='Obsoleto']['Quantity'].sum())
        exc_h  = int(r_hn_q[r_hn_q['Clasificacion']=='Exceso']['Quantity'].sum())
        wip_h  = int(r_hn_q[r_hn_q['Clasificacion'].isin(HN_WIP_CLAS)]['Quantity'].sum())
        k1,k2,k3,k4,k5,k6 = st.columns(6)
        with k1: st.markdown(kpi_card("Total", fmtk(tot_all_h), f"{len(r_hn_q):,} cajas"), unsafe_allow_html=True)
        with k2: st.markdown(kpi_card("Regular", fmtk(reg_h), f"{reg_h/max(tot_all_h,1)*100:.1f}%", INDIGO, INDIGO), unsafe_allow_html=True)
        with k3: st.markdown(kpi_card("VMI", fmtk(vmi_h), f"{vmi_h/max(tot_all_h,1)*100:.1f}%", INDIGO2, INDIGO2), unsafe_allow_html=True)
        with k4: st.markdown(kpi_card("Irregulares", fmtk(irr_h), f"{irr_h/max(tot_all_h,1)*100:.1f}%",'#F59E0B','#F59E0B'), unsafe_allow_html=True)
        with k5: st.markdown(kpi_card("Obsoleto+Exceso", fmtk(obs_h+exc_h), f"{(obs_h+exc_h)/max(tot_all_h,1)*100:.1f}% — atención",'#EF4444','#EF4444'), unsafe_allow_html=True)
        with k6: st.markdown(kpi_card("Wip", fmtk(wip_h), f"{wip_h/max(tot_all_h,1)*100:.1f}%", SLATE,'#64748B'), unsafe_allow_html=True)
...
with tab_tlp:
    if r_tlp is None:
        ...
        # Always calculate from full r_tlp so cards always show all classifications
        r_tlp_q = r_tlp.copy(); r_tlp_q['Quantity'] = pd.to_numeric(r_tlp_q['Quantity'], errors='coerce').fillna(0)
        sinc_t = int(r_tlp_q[r_tlp_q['Clasificacion']=='Sin Clasificacion']['Quantity'].sum())
        irr_t  = int(r_tlp_q[r_tlp_q['Clasificacion']=='Irregulares']['Quantity'].sum())
        exb_t  = int(r_tlp_q[r_tlp_q['Clasificacion']=='Exceso Blanks']['Quantity'].sum())
        exp_t  = int(r_tlp_q[r_tlp_q['Clasificacion']=='Exceso Printed']['Quantity'].sum())
        wip_t  = int(r_tlp_q[r_tlp_q['Clasificacion'].isin(TLP_WIP_CLAS)]['Quantity'].sum())
        tot_all_t = int(r_tlp_q['Quantity'].sum())
        k1,k2,k3,k4,k5,k6 = st.columns(6)
        with k1: st.markdown(kpi_card("Total", fmtk(tot_all_t), f"{len(r_tlp_q):,} cajas"), unsafe_allow_html=True)
        with k2: st.markdown(kpi_card("Sin Clasificacion", fmtk(sinc_t), f"{sinc_t/max(tot_all_t,1)*100:.1f}%", INDIGO, INDIGO), unsafe_allow_html=True)
        with k3: st.markdown(kpi_card("Irregulares", fmtk(irr_t), f"{irr_t/max(tot_all_t,1)*100:.1f}%",'#F59E0B','#F59E0B'), unsafe_allow_html=True)
        with k4: st.markdown(kpi_card("Exceso Blanks", fmtk(exb_t), f"{exb_t/max(tot_all_t,1)*100:.1f}%",'#F97316','#F97316'), unsafe_allow_html=True)
        with k5: st.markdown(kpi_card("Exceso Printed", fmtk(exp_t), f"{exp_t/max(tot_all_t,1)*100:.1f}%",'#EF4444','#EF4444'), unsafe_allow_html=True)
        with k6: st.markdown(kpi_card("Wip", fmtk(wip_t), f"{wip_t/max(tot_all_t,1)*100:.1f}%", SLATE,'#64748B'), unsafe_allow_html=True)
