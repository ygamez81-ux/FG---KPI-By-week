...
with tab_hn:
    if r_hn is None:
        ...
        with k2: st.markdown(kpi_card("Finished Goods", fmtk(fg_h), f"{fg_h/max(tot_h,1)*100:.1f}%",'#10B981','#10B981'), unsafe_allow_html=True)
        with k3: st.markdown(kpi_card("Wip", fmtk(wip_h), f"{wip_h/max(tot_h,1)*100:.1f}%", SLATE,'#64748B'), unsafe_allow_html=True)
        with k4: st.markdown(kpi_card("Irregulares", fmtk(irr_h), f"{irr_h/max(tot_h,1)*100:.1f}%",'#F59E0B','#F59E0B'), unsafe_allow_html=True)
        with k5: st.markdown(kpi_card("Obsoleto + Exceso", fmtk(crit_h), f"{crit_h/max(tot_h,1)*100:.1f}% — atención",'#EF4444','#EF4444'), unsafe_allow_html=True)
...
with tab_tlp:
    if r_tlp is None:
        ...
        if vmap2[view_t] == 'all':
            ...
            with k4: st.markdown(kpi_card("Exceso Blanks+Printed", fmtk(crit_t), f"{crit_t/max(tot_t,1)*100:.1f}% — atención",'#EF4444','#EF4444'), unsafe_allow_html=True)
            with k5: st.markdown(kpi_card("Wip", fmtk(wip_t), f"{wip_t/max(tot_t,1)*100:.1f}%", SLATE,'#64748B'), unsafe_allow_html=True)
        elif vmap2[view_t] == 'fg':
            sinc_t = int(df_t[df_t['Clasificacion']=='Sin Clasificacion']['Quantity'].sum())
            irr_t  = int(df_t[df_t['Clasificacion']=='Irregulares']['Quantity'].sum())
            crit_t = int(df_t[df_t['Clasificacion'].isin(['Exceso Blanks','Exceso Printed'])]['Quantity'].sum())
            with k2: st.markdown(kpi_card("Sin Clasificacion", fmtk(sinc_t), f"{sinc_t/max(tot_t,1)*100:.1f}%", INDIGO, INDIGO), unsafe_allow_html=True)
            with k3: st.markdown(kpi_card("Irregulares", fmtk(irr_t), f"{irr_t/max(tot_t,1)*100:.1f}%",'#F59E0B','#F59E0B'), unsafe_allow_html=True)
            with k4: st.markdown(kpi_card("Exceso Blanks+Printed", fmtk(crit_t), f"{crit_t/max(tot_t,1)*100:.1f}% — atención",'#EF4444','#EF4444'), unsafe_allow_html=True)
            with k5: st.markdown(kpi_card("Cajas", f"{caj_t:,}", "Finished Goods", INDIGO, INDIGO), unsafe_allow_html=True)
        else:  # wip
            wip_t  = int(df_t[df_t['Clasificacion'].isin(TLP_WIP_CLAS)]['Quantity'].sum())
            with k2: st.markdown(kpi_card("Wip", fmtk(wip_t), f"{wip_t/max(tot_t,1)*100:.1f}%", SLATE,'#64748B'), unsafe_allow_html=True)
            with k3: st.markdown(kpi_card("Cajas", f"{caj_t:,}", "en Wip", SLATE,'#64748B'), unsafe_allow_html=True)
            with k4: st.markdown(kpi_card("% del total", f"{wip_t/max(int(r_tlp['Quantity'].sum()),1)*100:.1f}%", "vs inventario completo", SLATE,'#64748B'), unsafe_allow_html=True)
            with k5: st.markdown(kpi_card("Clasificaciones", "1", "Wip", SLATE,'#64748B'), unsafe_allow_html=True)
